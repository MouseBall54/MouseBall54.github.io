...
layout: single
title: >
  Claude Code Permission Modes: plan, acceptEdits, auto, and bypassPermissions
seo_title: >
  Claude Code Permission Modes: plan, acceptEdits, auto, and bypassPer
...
Use `plan` for unfamiliar repos, `acceptEdits` for normal work, reserve `auto` for trusted setups, and disable bypass where policy matters.
...
![Claude Code Permission Modes: plan, acceptEdits, auto, and bypassPermissions workflow diagram](/images/2026-03-03-claude-code-permission-modes/hero.svg)

## When This Setup Matters

Claude Code permission mode decides whether a session plans, accepts edits, auto-approves, or bypasses prompts. This matters when a developer wants repeatable AI-agent help instead of a long ad hoc prompt. A good setup makes three things visible: what the agent may read, what it may change, and how the human will verify the result.

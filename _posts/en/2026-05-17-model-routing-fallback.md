...
layout: single
title: >
  Model Routing and Fallback: When to Use Cheap or Strong Models
seo_title: >
  Model Routing and Fallback: When to Use Cheap or Strong Models
...
header:
  ...
  overlay_filter: 0.34
  image_description: >
    Visual summary of the verification flow and practical checkpoints for Model Routing and Fallback: When to Use Cheap or Strong Models.
excerpt: >
...
This guide treats **Model Routing and Fallback: When to Use Cheap or Strong Models** as a practical checklist rather than a headline. The useful move is to track `routing rule` and `quality floor` together, then separate conditions that require more review from conditions that require action.

This is an educational workflow guide, not a recommendation for a specific model or vendor.

![Model Routing and Fallback: When to Use Cheap or Strong Models core workflow diagram](/images/expert-growth-model-routing-fallback/hero.svg)
...
- **Primary signal**: Track `routing rule` with date, source, and owner instead of as an isolated number.
- **Secondary signal**: Mark whether a change in `quality floor` should reopen the conclusion.
...
![Model Routing and Fallback: When to Use Cheap or Strong Models practical checklist](/images/expert-growth-model-routing-fallback/checklist.svg)

## Practical Workflow

1. Write the current problem in one sentence, such as “we are delayed because `routing rule` is unclear.”
...
| --- | --- | --- |
| Primary signal | Current state of `routing rule` | Prevents headline-only decisions |
| Secondary signal | Direction of `quality floor` | Shows when the conclusion can change |
...
No. `routing rule` and `quality floor` can change meaning as rules, data, costs, or user behavior change. A quarterly review is a practical minimum for most teams.

...
layout: single
title: >
  Claude Code permission modes: plan, acceptEdits, auto, bypassPermissions 차이
seo_title: >
  Claude Code permission modes: plan, acceptEdits, auto, bypassPermiss
date: 2026-03-03T09:24:00+09:00
last_modified_at: 2026-05-24T09:30:00+09:00
lang: ko
translation_id: ai-agent-cli-claude-code-permission-modes
header:
  teaser: /images/2026-03-03-claude-code-permission-modes/hero.svg
  overlay_image: /images/2026-03-03-claude-code-permission-modes/hero.svg
  overlay_filter: 0.45
  image_description: >
...
새 저장소는 `plan`, 일반 수정은 `acceptEdits`, 자동 모드는 신뢰된 환경에서만 사용하고 `bypassPermissions`는 조직 정책으로 막는 것을 기본값으로 둡니다.

실무 원칙은 단순합니다. 에이전트 권한은 작업 범위보다 좁아야 합니다. 설치, 설정, MCP 도구, 저장소 지침은 일회성 준비가 아니라 코드 품질을 지키는 운영 장치로 봐야 합니다.

![Claude Code permission modes: plan, acceptEdits, auto, bypassPermissions 차이 workflow diagram](/images/2026-03-03-claude-code-permission-modes/hero.svg)

## 언제 중요한가

Claude Code permission mode는 세션이 계획만 할지, 편집을 받을지, 자동 승인할지, 위험한 우회를 허용할지 정한다. 이 설정은 긴 프롬프트 하나로 모든 일을 맡기려는 상황보다, 반복 가능한 AI agent 작업 흐름을 만들 때 중요합니다. 좋은 설정은 세 가지를 분명히 합니다. agent가 읽어도 되는 것, 바꿔도 되는 것, 사람이 검증할 방법입니다.
...
- start low-trust work in plan mode.
- log mode changes in handoff notes.
- deny secrets regardless of mode.

설정이 동작하면 바로 편집을 맡기지 말고 먼저 읽기 전용 요약을 요청하세요. 그다음 좁은 계획을 요청합니다. 두 응답이 저장소 현실과 맞을 때만 파일 수정이나 외부 도구 호출을 허용하는 편이 안전합니다.

## 흔한 실수

- using bypass for convenience.
- letting repositories set risky local defaults.
- forgetting CLI flags override settings for one session.
...
**Claude Code permission modes: plan, acceptEdits, auto, bypassPermissions 차이**에서 중요한 기준은 독자가 한 번 따라 해서 성공했는지가 아닙니다. 이 주제는 AI 거버넌스와 워크플로 의사결정로 다루는 편이 안전합니다. 결론을 내리기 전에 작업 경계, 평가 데이터, 사람 검토 조건, 비용과 지연시간 예산를 확인해야 합니다. 또한 나중에 같은 문제가 반복될 수 있으므로, 관찰한 사실과 사용한 가정, 결론이 바뀔 조건을 짧은 결정 기록으로 남기는 것이 좋습니다.
